...
#define ALEX_DATA_NODE_SEP_ARRAYS 0
...
// A parent class for both types of ALEX nodes
    ...
    public:
        ...
        BlockNumber alexInitNodePage(Relation index) {

//            elog(INFO, "==================alexInitNodePage=================");
//            std::cout << "=========================================" << this << " alexInitNodePage================="
//                      << std::endl;
...
//            std::cout << "new blockNumber: " << blockNumber << std::endl;

            /* Initialize contents of page */
            state = GenericXLogStart(index);
//            std::cout << "state before: " << state << std::endl;
            page = GenericXLogRegisterBuffer(state, buffer, GENERIC_XLOG_FULL_IMAGE);
            this->alexFillNodePage(index, page, blockNumber);
...
//            std::cout << "=========================================" << this << " alexInitNodePage end================="
//                      << std::endl;
        ...
        virtual void
        alexFillNodePage(Relation index, Page nodePage, BlockNumber blockNumber) = 0;
    ...
    public:
        ...
        // only used in psql scan，用于 children 的按需读取
        BlockNumber *childrenBlockNumber = nullptr;
        ...
        ~AlexModelNode() {
//            std::cout << "~AlexModelNode" << std::endl;
            if (children_ != nullptr) {
                pointer_allocator().deallocate(children_, num_children_);
            }
            if (childrenBlockNumber != nullptr) {
                delete[] childrenBlockNumber;
            }
//            std::cout << "here4" << std::endl;
        ...
        BlockNumber alexInitChildren(Relation index) {

//            elog(INFO, "==================alexInitChildren=================");
//            std::cout << "==================" << this << " alexInitChildren=================" << std::endl;
            ...
            // 每个 block 要放的 children 的索引
            std::vector <std::vector<int>> vv;
            int cur = 0;
            std::vector<int> v;
            while (cur < this->num_children_) {
                v.push_back(cur);
                cur++;
                if (v.size() == MaxBlockNumberPerBlock) {
                    vv.push_back(v);
                    v.clear();
                }
            }
...
//                std::cout << "new blockNumber: " << blockNumber << std::endl;
                ...
                // 初始化 page 的 header 和 special 部分
                BloomInitPage(page, 0, prev);
                prev = blockNumber;

                // 得到 page 的 content 部分
                uint8_t *pageData = AlexGetGeneralContent(page);
                memset(pageData, 0, vv[i].size() * sizeof(BlockNumber));

                // 逐个设置 BlockNumber
                for (int j = 0; j < (int) (vv[i].size()); ++j) {
                    BlockNumber no = nodeBlockNumberMapper[this->children_[vv[i][j]]];
                    BlockNumber *tmp = &no;
                    memcpy(pageData, tmp, sizeof(BlockNumber));
                    pageData += sizeof(BlockNumber);
                }

                // 调整 pd_lower
                ((PageHeader) page)->pd_lower = pageData - (uint8_t *) page;
        ...
        void alexFillNodePage(Relation index, Page nodePage, BlockNumber blockNumber) {

//            elog(INFO, "==================AlexFillModelNodePage=================");
//            std::cout << "==================" << this << " AlexFillModelNodePage for blockNumber " << blockNumber
//                      << "=================" << std::endl;

            AlexModelNodePageData *pageData;

            BloomInitPage(nodePage, ALEX_MODEL_NODE);

            pageData = AlexGetModelPageContent(nodePage);
            memset(pageData, 0, sizeof(AlexModelNodePageData));

            pageData->is_leaf_ = this->is_leaf_;
            pageData->duplication_factor_ = this->duplication_factor_;
            pageData->level_ = this->level_;
            pageData->model_a = this->model_.a_;
            pageData->model_b = this->model_.b_;
            pageData->cost_ = this->cost_;
            pageData->num_children_ = this->num_children_;
//            std::cout << "this->num_children_: " << this->num_children_ << std::endl;
            pageData->children_ = InvalidBlockNumber;

            ((PageHeader) nodePage)->pd_lower += sizeof(AlexModelNodePageData);

            /* If this fails, probably FreeBlockNumberArray size calc is wrong: */
            Assert(((PageHeader) nodePage)->pd_lower <= ((PageHeader) nodePage)->pd_upper);
...
*/
    ...
    public:
        ...
        BlockNumber nextLeafBlockNumber = InvalidBlockNumber;
        self_type *prev_leaf_ = nullptr;
        BlockNumber prevLeafBlockNumber = InvalidBlockNumber;
        ...
        // 一个数据节点最大为 16MB，一个 <int, ItemPointerData> 的大小为 4+6=10B，所以最多存 1677721 个 pair
        // 一个 page 为 8KB，那么一个满的数据节点需要 2048 个 page 来存
        ...
        static constexpr T
        kEndSentinel_ = std::numeric_limits<T>::max();
        ...
        ~AlexDataNode() {
//            std::cout << "~AlexDataNode" << std::endl;
#if ALEX_DATA_NODE_SEP_ARRAYS
            //            elog(INFO, "==================~AlexDataNode()=================");
        ...
        BlockNumber alexInitBitmap(Relation index, int &compressedSize) {

//            elog(INFO, "==================alexInitBitmap=================");
//            std::cout << "==================" << this << " alexInitBitmap=================" << std::endl;


            // 对bitmap进行压缩
//            std::cout << "bitmapSize(64b) = " << bitmap_size_ << std::endl;
            std::vector<unsigned char> compressedBitmap = compressBitmap();
//            std::cout << "compressedBitmap.size(B) = " << compressedBitmap.size() << std::endl;
            compressedSize = compressedBitmap.size();
            ...
            // 每个 block 要放的 char 的索引
            std::vector <std::vector<int>> vv;
            int cur = 0;
            std::vector<int> v;
            while (cur < (int) compressedBitmap.size()) {
                v.push_back(cur);
                cur++;
                if (v.size() == MaxCharPerBlock) {
                    vv.push_back(v);
                    v.clear();
                }
            }
...
//                std::cout << "new blockNumber: " << blockNumber << std::endl;
                ...
                // 得到 page 的 content 部分
                uint8_t *pageData = AlexGetGeneralContent(page);
                memset(pageData, 0, vv[i].size() * sizeof(unsigned char));

                // 逐个设置 char
                for (int j = 0; j < (int) (vv[i].size()); ++j) {
                    unsigned char item = compressedBitmap[vv[i][j]];
                    unsigned char *tmp = &item;
                    memcpy(pageData, tmp, sizeof(unsigned char));
                    pageData += sizeof(unsigned char);
                }
        ...
        BlockNumber alexInitSlots(Relation index) {

//            elog(INFO, "==================alexInitSlots=================");
//            std::cout << "==================" << this << " alexInitSlots=================" << std::endl;
            ...
            // 每个 block 要放的 slots 的索引
            std::vector <std::vector<int>> vv;
            int cur = 0;
            std::vector<int> v;
            while (cur < this->data_capacity_) {
                v.push_back(cur);
                cur++;
                if (v.size() == MaxDataSlotPerBlock) {
                    vv.push_back(v);
                    v.clear();
                }
            }
...
//                std::cout << "vv[" << i << "].size() = " << vv[i].size() << std::endl;
...
//                std::cout << "here2" << std::endl;
                ...
                // 得到 page 的 content 部分
                uint8_t *pageData = AlexGetGeneralContent(page);
                memset(pageData, 0, vv[i].size() * sizeof(V));
...
//                std::cout << "here3" << std::endl;
                ...
                for (int j = 0; j < (int) (vv[i].size()); ++j) {
                    DataSlotItem item;
                    item.key = this->data_slots_[vv[i][j]].first;
                    item.value = this->data_slots_[vv[i][j]].second;
                    DataSlotItem *tmp = &item;
                    memcpy(pageData, tmp, sizeof(DataSlotItem));
                    pageData += sizeof(DataSlotItem);
                }
...
//            std::cout << "alexInitSlots end" << std::endl;
...
//            elog(INFO, "==================AlexFillDataNodePage=================");
//            std::cout << "==================" << this << " AlexFillDataNodePage for blockNumber " << blockNumber
//                      << "=================" << std::endl;

            AlexDataNodePageData *pageData;

            BloomInitPage(nodePage, ALEX_DATA_NODE);

            pageData = AlexGetDataPageContent(nodePage);
            memset(pageData, 0, sizeof(AlexDataNodePageData));
            ...
            pageData->model_b = this->model_.b_;
            pageData->cost_ = this->cost_;
            pageData->next_leaf_ = InvalidBlockNumber;
            pageData->prev_leaf_ = InvalidBlockNumber;
            pageData->data_capacity_ = this->data_capacity_;
//            std::cout << "this->data_capacity_: " << this->data_capacity_ << std::endl;
            pageData->num_keys_ = this->num_keys_;
//            std::cout << "this->num_keys_: " << this->num_keys_ << std::endl;
            pageData->bitmap_size_ = this->bitmap_size_;
//            std::cout << "this->bitmap_size_: " << this->bitmap_size_ << std::endl;
            pageData->expansion_threshold_ = this->expansion_threshold_;
            pageData->contraction_threshold_ = this->contraction_threshold_;
            pageData->max_slots_ = this->max_slots_;
//            std::cout << "this->max_slots_: " << this->max_slots_ << std::endl;
            pageData->num_shifts_ = this->num_shifts_;
            pageData->num_exp_search_iterations_ = this->num_exp_search_iterations_;
            pageData->num_lookups_ = this->num_lookups_;
            pageData->num_inserts_ = this->num_inserts_;
            pageData->num_resizes_ = this->num_resizes_;
            pageData->max_key_ = this->max_key_;
            pageData->min_key_ = this->min_key_;
            pageData->num_right_out_of_bounds_inserts_ = this->num_right_out_of_bounds_inserts_;
            pageData->num_left_out_of_bounds_inserts_ = this->num_left_out_of_bounds_inserts_;
            pageData->expected_avg_exp_search_iterations_ = this->expected_avg_exp_search_iterations_;
            pageData->expected_avg_shifts_ = this->expected_avg_shifts_;
            // 这个 BlockNumber 可以一开始就写好，因为不涉及节点
            pageData->data_slots_ = alexInitSlots(index);
            int compressedSize = 0;
            pageData->bitmap_ = alexInitBitmap(index, compressedSize);
            pageData->compressed_bitmap_size_ = compressedSize;

//            std::cout << "pd_lower: " << ((PageHeader) nodePage)->pd_lower << std::endl;
//            std::cout << "pd_upper: " << ((PageHeader) nodePage)->pd_upper << std::endl;
//            std::cout << "compressedBitmap.size(): " << compressedBitmap.size() << std::endl;
//            std::cout << "sizeof(unsigned char): " << sizeof(unsigned char) << std::endl;
//            std::cout << "sizeof(AlexDataNodePageData): " << sizeof(AlexDataNodePageData) << std::endl;
            ((PageHeader) nodePage)->pd_lower += sizeof(AlexDataNodePageData);
...
//            std::cout << "here14" << std::endl;
        ...
        // 根据node目前的bitmap和bitmapSize进行压缩
        std::vector<unsigned char> compressBitmap() {
            uLongf compressedBufferSize = compressBound(this->bitmap_size_ * sizeof(uint64_t));
            std::vector<unsigned char> compressedBuffer(compressedBufferSize);
            if (compress2(compressedBuffer.data(),
                          &compressedBufferSize,
                          reinterpret_cast<const Bytef *>(this->bitmap_),
                          this->bitmap_size_ * sizeof(uint64_t),
                          Z_BEST_COMPRESSION)
                == Z_OK) {
...
//                std::cout << "压缩失败" << std::endl;
        ...
        // 根据参数给出的压缩后的数据以及node目前存储的bitmapSize，复原bitmap
        void decompress(const unsigned char *compressedData, int compressedSize) {
            uLongf decompressedBufferSize = this->bitmap_size_ * sizeof(uint64_t);
            std::vector <uint64_t> *decompressedBuffer = new std::vector<uint64_t>(this->bitmap_size_);;
            int result = uncompress(reinterpret_cast<Bytef *>(decompressedBuffer->data()), &decompressedBufferSize,
                                    compressedData, compressedSize);
            if (result == Z_OK) {
                this->bitmap_ = decompressedBuffer->data();
            } else {
//                std::cout << "解压失败" << std::endl;
        ...
        inline T &get_key_psql(IndexScanDesc scan, int pos) const {
        ...
        inline bool check_exists_psql(IndexScanDesc scan, int pos) const {
            assert(pos >= 0 && pos < data_capacity_);
            int bitmap_pos = pos >> 6;
            int bit_pos = pos - (bitmap_pos << 6);
            return static_cast<bool>(bitmap_[bitmap_pos] & (1ULL << bit_pos));
        ...
        T first_key_psql(IndexScanDesc scan) const {
            for (int i = 0; i < data_capacity_; i++) {
                if (check_exists_psql(scan, i)) return get_key_psql(scan, i);
            }
            return std::numeric_limits<T>::max();
        }
        ...
        T last_key_psql(IndexScanDesc scan) const {
            for (int i = data_capacity_ - 1; i >= 0; i--) {
                if (check_exists_psql(scan, i)) return get_key_psql(scan, i);
            }
            return std::numeric_limits<T>::lowest();
            ...
            while (true) {
                ...
                if (expected_insert_frac == 0) {
                    ...
                    build_node_implicit_sampling(values, num_keys, sample_num_keys,
                                                 ...
                                                 &sample_model);
                    SampleDataNodeStats temp_stats{std::log2(sample_num_keys), acc.get_stat(), 0};
                    sample_stats.push_back(temp_stats);
                } else {
                    ExpectedIterationsAndShiftsAccumulator acc(sample_data_capacity);
                    build_node_implicit_sampling(values, num_keys, sample_num_keys,
                                                 sample_data_capacity, step_size, &acc,
                                                 &sample_model);
                    SampleDataNodeStats temp_stats{std::log2(sample_num_keys), acc.get_expected_num_search_iterations(),
...
#else
                new_data_slots[i].first = kEndSentinel_;
#endif
            }
