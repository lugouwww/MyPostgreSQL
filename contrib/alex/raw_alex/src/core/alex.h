...
namespace alex {
    ...
    public:
        ...
        typedef AlexNode<T, P> node_type;
...
//            elog(INFO, "==================BloomInitMetapage=================");
//            std::cout << "==================BloomInitMetapage=================" << std::endl;
...
//            elog(INFO, "==================BloomFillMetapage=================");
            ...
            metadata->key_domain_min_ = istats_.key_domain_min_;
            metadata->key_domain_max_ = istats_.key_domain_max_;
            metadata->num_keys_above_key_domain = istats_.num_keys_above_key_domain;
            metadata->num_keys_below_key_domain = istats_.num_keys_below_key_domain;
            metadata->num_keys_at_last_right_domain_resize = istats_.num_keys_at_last_right_domain_resize;
            metadata->num_keys_at_last_left_domain_resize = istats_.num_keys_at_last_left_domain_resize;
            metadata->opts = *opts;
        ...
        // 用于psql的初始构造函数，albuild时使用 myfind
        Alex(Relation heap, Relation index, IndexInfo *indexInfo, IndexBuildResult *result) {

//            elog(INFO, "==================Alex(IndexBuildResult *result)=================");
//            std::cout << "==================Alex(IndexBuildResult *result)=================" << result << std::endl;

//            std::cout << "MaxBlockNumberPerBlock: " << MaxBlockNumberPerBlock << std::endl;

//            // 2
//            std::cout << sizeof(OffsetNumber) << std::endl;
//            // 4
//            std::cout << sizeof(BlockNumber) << std::endl;

            if (RelationGetNumberOfBlocks(index) != 0)
                elog(ERROR, "index \"%s\" already contains data", RelationGetRelationName(index));

//            std::cout << "RelationGetNumberOfBlocks(index) == 0" << std::endl;

            double reltuples;
            BloomBuildState buildstate;

            // 先扫描表中现有的数据，存储到临时变量callbackTemp中
            reltuples = table_index_build_scan(heap, index, indexInfo, true, true, bloomBuildCallback,
                                               nullptr, NULL);
//            std::cout << "reltuples = " << reltuples << std::endl;
//            for (const auto &kv: callbackTemp) {
//                std::cout << kv.first << std::endl;
//            }

            // 排序后进行bulk_load
            std::sort(callbackTemp.begin(), callbackTemp.end(),
                      [this](auto const &a, auto const &b) {
                          return key_less_(a.first, b.first);
                      });
            bulk_load(callbackTemp.data(), static_cast<int>(callbackTemp.size()));
            callbackTemp.clear();
//            std::cout << "super = " << superroot_ << std::endl;
//            std::cout << "root = " << root_node_ << std::endl;
//            printf("super: a = %f, b = %f\n", superroot_->model_.a_, superroot_->model_.b_);
//            printf("root: a = %f, b = %f\n", root_node_->model_.a_, root_node_->model_.b_);

            // -------- 此时内存中的alex已经布局好了，需要写page了 --------
...
//            std::cout << "write page begin!!" << std::endl;
            ...
            // 不递归，用迭代，迭代过程中，对于 next prev children 这三个需要填写 BlockNumber 的，page 里先空着，最后再补上
            BlockNumber blkno = superroot_->alexInitNodePage(index);
            nodeBlockNumberMapper.insert({superroot_, blkno});
            for (NodeIterator node_it = NodeIterator(this); !node_it.is_end(); node_it.next()) {
                blkno = node_it.current()->alexInitNodePage(index);
                auto it = nodeBlockNumberMapper.find(node_it.current());
                if (it != nodeBlockNumberMapper.end()) {
                    std::cout << "!!error，不应该运行到这里" << std::endl;
                }
                nodeBlockNumberMapper.insert({node_it.current(), blkno});
            }

            // 补上
            setBlockNumbersInPage(index, superroot_);
            for (NodeIterator node_it = NodeIterator(this); !node_it.is_end(); node_it.next()) {
                auto it = nodeBlockNumberMapper.find(node_it.current());
                if (it == nodeBlockNumberMapper.end()) {
                    std::cout << "!!error，不应该运行到这里" << std::endl;
                }
                setBlockNumbersInPage(index, node_it.current());
            }
...
//            std::cout << "write end!!" << std::endl;
            ...
            elog(INFO, "model block count = %d", modelBlockCount);


//            std::cout << this->istats_.key_domain_min_ << std::endl;
//            std::cout << this->istats_.key_domain_max_ << std::endl;
//            std::cout << this->istats_.num_keys_below_key_domain << std::endl;
//            std::cout << this->istats_.num_keys_above_key_domain << std::endl;
//            std::cout << this->istats_.num_keys_at_last_right_domain_resize << std::endl;
//            std::cout << this->istats_.num_keys_at_last_left_domain_resize << std::endl;
//
//            std::cout << "root node info: is_leaf(" << root_node_->is_leaf_ << "), level(" << root_node_->level_ << ")"
//                      << std::endl;
//            printf("super: a = %f, b = %f\n", superroot_->model_.a_, superroot_->model_.b_);
//            printf("root: a = %f, b = %f\n", root_node_->model_.a_, root_node_->model_.b_);
        ...
        node_type *readNode(IndexScanDesc scan, BlockNumber blkno) {

//            std::cout << "=====readNode: " << blkno << std::endl;
//
//            for (auto it = nodesRead.begin(); it != nodesRead.end(); it++) {
//                std::cout << it->first << ":" << it->second << std::endl;
//            }

            auto it = nodesRead.find(blkno);
            if (it != nodesRead.end()) {
//                std::cout << "find " << blkno << " in nodesRead" << std::endl;
                return (node_type *) (it->second);
            }

            Buffer buffer = ReadBufferExtended(scan->indexRelation, MAIN_FORKNUM, blkno, RBM_NORMAL, nullptr);
            ...
            // model node
            if (opaque->flags == ALEX_MODEL_NODE) {

//                std::cout << "ALEX_MODEL_NODE" << std::endl;

                model_node_type *node = new(model_node_allocator().allocate(1)) model_node_type();

                AlexModelNodePageData *modelPageData = AlexGetModelPageContent(page);

                node->is_leaf_ = modelPageData->is_leaf_;
                node->duplication_factor_ = modelPageData->duplication_factor_;
                node->level_ = modelPageData->level_;
                node->model_.a_ = modelPageData->model_a;
                node->model_.b_ = modelPageData->model_b;
                node->cost_ = modelPageData->cost_;
                node->num_children_ = modelPageData->num_children_;
                node->children_ = new(node->pointer_allocator().allocate(node->num_children_))
                        AlexNode<T, P> *[node->num_children_];
                for (int i = 0; i < node->num_children_; ++i) {
                    node->children_[i] = nullptr;
                }

                BlockNumber childrenBlkno = modelPageData->children_;

                UnlockReleaseBuffer(buffer);
                CHECK_FOR_INTERRUPTS();

//                std::cout << "basic attr read over" << std::endl;
//
//                std::cout << "begin handle childrenBlockNumber info" << std::endl;

                node->childrenBlockNumber = new BlockNumber[node->num_children_];
                int idx = 0;

                while (true) {

                    // children
                    buffer = ReadBufferExtended(scan->indexRelation, MAIN_FORKNUM, childrenBlkno, RBM_NORMAL, nullptr);
                    LockBuffer(buffer, BUFFER_LOCK_SHARE);
                    page = BufferGetPage(buffer);
                    TestForOldSnapshot(scan->xs_snapshot, scan->indexRelation, page);

                    BlockNumber *childrenData = (BlockNumber * )AlexGetGeneralContent(page);
                    for (int i = 0; i < (int) MaxBlockNumberPerBlock && idx < node->num_children_; ++i) {
                        node->childrenBlockNumber[idx] = *childrenData;
                        childrenData++;
                        idx++;
                    }

                    bool canBreak = true;
                    if (idx < node->num_children_) {
                        opaque = BloomPageGetOpaque(page);
                        if (opaque->nextBlockNumber != InvalidBlockNumber) {
                            childrenBlkno = opaque->nextBlockNumber;
                            canBreak = false;
                        }
                    }
            ...
            // data node
//            std::cout << "ALEX_DATA_NODE" << std::endl;

            data_node_type *node = new(data_node_allocator().allocate(1)) data_node_type();

            AlexDataNodePageData *dataPageData = AlexGetDataPageContent(page);

            node->is_leaf_ = dataPageData->is_leaf_;
            node->duplication_factor_ = dataPageData->duplication_factor_;
            node->level_ = dataPageData->level_;
            node->model_.a_ = dataPageData->model_a;
            node->model_.b_ = dataPageData->model_b;
            node->cost_ = dataPageData->cost_;
            node->data_capacity_ = dataPageData->data_capacity_;
            node->num_keys_ = dataPageData->num_keys_;
            node->bitmap_size_ = dataPageData->bitmap_size_;
            node->expansion_threshold_ = dataPageData->expansion_threshold_;
            node->contraction_threshold_ = dataPageData->contraction_threshold_;
            node->max_slots_ = dataPageData->max_slots_;
            node->num_shifts_ = dataPageData->num_shifts_;
            node->num_exp_search_iterations_ = dataPageData->num_exp_search_iterations_;
            node->num_lookups_ = dataPageData->num_lookups_;
            node->num_inserts_ = dataPageData->num_inserts_;
            node->num_resizes_ = dataPageData->num_resizes_;
            node->max_key_ = dataPageData->max_key_;
            node->min_key_ = dataPageData->min_key_;
            node->num_right_out_of_bounds_inserts_ = dataPageData->num_right_out_of_bounds_inserts_;
            node->num_left_out_of_bounds_inserts_ = dataPageData->num_left_out_of_bounds_inserts_;
            node->expected_avg_exp_search_iterations_ = dataPageData->expected_avg_exp_search_iterations_;
            node->expected_avg_shifts_ = dataPageData->expected_avg_shifts_;
            node->prevLeafBlockNumber = dataPageData->prev_leaf_;
            node->nextLeafBlockNumber = dataPageData->next_leaf_;
            BlockNumber dataSlotsBlkno = dataPageData->data_slots_;
            int compressedBitmapSize = dataPageData->compressed_bitmap_size_;
            BlockNumber bitmapBlkno = dataPageData->bitmap_;
...
//            std::cout << "begin handle bitmap info" << std::endl;

            int idx = 0;
            unsigned char compressedBitmap[compressedBitmapSize];

            while (true) {

                buffer = ReadBufferExtended(scan->indexRelation, MAIN_FORKNUM, bitmapBlkno, RBM_NORMAL, nullptr);
                LockBuffer(buffer, BUFFER_LOCK_SHARE);
                page = BufferGetPage(buffer);
                TestForOldSnapshot(scan->xs_snapshot, scan->indexRelation, page);

                unsigned char *bitmapData = (unsigned char *) AlexGetGeneralContent(page);
                for (int i = 0; i < (int) MaxCharPerBlock && idx < compressedBitmapSize; ++i) {
                    compressedBitmap[idx] = *bitmapData;
                    bitmapData++;
                    idx++;
                }

                bool canBreak = true;
                if (idx < compressedBitmapSize) {
                    opaque = BloomPageGetOpaque(page);
                    if (opaque->nextBlockNumber != InvalidBlockNumber) {
                        bitmapBlkno = opaque->nextBlockNumber;
                        canBreak = false;
                    }
                }
            ...
            node->decompress(compressedBitmap, compressedBitmapSize);

//            std::cout << "begin handle dataSlots block: " << dataSlotsBlkno << std::endl;

            node->data_slots_ = new(node->value_allocator().allocate(node->data_capacity_))V[node->data_capacity_];
            idx = 0;

            while (true) {

                // data_slots_
                buffer = ReadBufferExtended(scan->indexRelation, MAIN_FORKNUM, dataSlotsBlkno, RBM_NORMAL, nullptr);
                LockBuffer(buffer, BUFFER_LOCK_SHARE);
                page = BufferGetPage(buffer);
                TestForOldSnapshot(scan->xs_snapshot, scan->indexRelation, page);

                DataSlotItem *slotsData = (DataSlotItem *) AlexGetGeneralContent(page);
                for (int i = 0; i < (int) MaxDataSlotPerBlock && idx < node->data_capacity_; ++i) {
                    node->data_slots_[idx].first = (*slotsData).key;
                    node->data_slots_[idx].second = (*slotsData).value;
                    slotsData++;
                    idx++;
                }

                bool canBreak = true;
                if (idx < node->data_capacity_) {
                    opaque = BloomPageGetOpaque(page);
                    if (opaque->nextBlockNumber != InvalidBlockNumber) {
                        dataSlotsBlkno = opaque->nextBlockNumber;
                        canBreak = false;
                    }
                }
        ...
        void setBlockNumbersInPage(Relation index, node_type *node) {

            BlockNumber blockNumber = nodeBlockNumberMapper[node];

            // 读到 node 对应的 page
            Buffer buffer = ReadBufferExtended(index, MAIN_FORKNUM, blockNumber, RBM_NORMAL, nullptr);
            ...
            // model node，需要设置 children
            // data node，需要设置 prev 和 next
            if (opaque->flags == ALEX_MODEL_NODE) {
                AlexModelNodePageData *pageData = AlexGetModelPageContent(page);
                modelBlockCount++;
                pageData->children_ = static_cast<model_node_type *>(node)->alexInitChildren(index);
            } else {
                AlexDataNodePageData *pageData = AlexGetDataPageContent(page);
                pageData->prev_leaf_ = nodeBlockNumberMapper[static_cast<data_node_type *>(node)->prev_leaf_];
                pageData->next_leaf_ = nodeBlockNumberMapper[static_cast<data_node_type *>(node)->next_leaf_];
            }
        ...
        // todo 假定只有 i=7 这一查询条件
        // todo 结构体的类型不能是指针不然没法复原
        // todo 还是先简单点，直接把整个 alex 复原再进行 scan，不要按需读取 block 了
        Alex(IndexScanDesc scan, TIDBitmap *tbm, int64 *ntids) {

//            elog(INFO, "===========Alex(IndexScanDesc scan, TIDBitmap *tbm)===========");
//            std::cout << "===========Alex(IndexScanDesc scan, TIDBitmap *tbm)===========" << std::endl;

            // meta
            BlockNumber blkno = BLOOM_METAPAGE_BLKNO;
            Buffer buffer = ReadBufferExtended(scan->indexRelation, MAIN_FORKNUM, blkno, RBM_NORMAL, nullptr);
            LockBuffer(buffer, BUFFER_LOCK_SHARE);
            Page page = BufferGetPage(buffer);
            TestForOldSnapshot(scan->xs_snapshot, scan->indexRelation, page);

            BloomMetaPageData *metaPageData = BloomPageGetMeta(page);
            this->istats_.key_domain_min_ = metaPageData->key_domain_min_;
            this->istats_.key_domain_max_ = metaPageData->key_domain_max_;
            this->istats_.num_keys_below_key_domain = metaPageData->num_keys_below_key_domain;
            this->istats_.num_keys_above_key_domain = metaPageData->num_keys_above_key_domain;
            this->istats_.num_keys_at_last_right_domain_resize = metaPageData->num_keys_at_last_right_domain_resize;
            this->istats_.num_keys_at_last_left_domain_resize = metaPageData->num_keys_at_last_left_domain_resize;
...
//            std::cout << this->istats_.num_keys_at_last_left_domain_resize << std::endl;
            ...
            // super
            superroot_ = static_cast<model_node_type *>(readNode(scan, BLOOM_SUPER_ROOT_BLKNO));
//            std::cout << "super" << std::endl;
//            for (int i = 0; i < superroot_->num_children_; ++i) {
//                std::cout << superroot_->childrenBlockNumber[i] << std::endl;
//            }

            // root
            root_node_ = readNode(scan, superroot_->childrenBlockNumber[0]);
//            std::cout << "root is_leaf and level: " << root_node_->is_leaf_ << " " << root_node_->level_ << std::endl;

//            printf("super: a = %f, b = %f\n", superroot_->model_.a_, superroot_->model_.b_);
//            printf("root: a = %f, b = %f\n", root_node_->model_.a_, root_node_->model_.b_);

            ScanKey scanKey = scan->keyData;
            // todo 虽然 scanKey 是一个结构体数组，但目前假定只有一个 attr，所以只关注 sk_argument 即可
//            std::cout << "scanKey: " << scanKey->sk_argument << std::endl;

            // todo 这里进行了两次 bound 操作，能否减少为一次？
            for (Iterator it = lower_bound_psql(scan, scanKey->sk_argument);
                 it != upper_bound_psql(scan, scanKey->sk_argument); it++) {
//                std::cout << "it.cur_leaf_ =  " << it.cur_leaf_ << std::endl;
                tbm_add_tuples(tbm, &(it.cur_leaf_->data_slots_[it.cur_idx_].second), 1, false);
                (*ntids)++;
            }
        ...
        ~Alex() {
            if (nodesRead.size() > 0) {
//                std::cout << "==================~Alex() nodesRead=================" << std::endl;
                for (auto it = nodesRead.begin(); it != nodesRead.end(); it++) {
//                    std::cout << "delete: " << (node_type *) (it->second) << std::endl;
                    delete_node((node_type *) (it->second));
                }
            } else {
//                std::cout << "==================~Alex() normal=================" << std::endl;
                for (NodeIterator node_it = NodeIterator(this); !node_it.is_end(); node_it.next()) {
//                    std::cout << "delete: " << node_it.current() << std::endl;
                    delete_node(node_it.current());
                }
//                std::cout << "delete super: " << superroot_ << std::endl;
                delete_node(superroot_);
            }
            nodeBlockNumberMapper.clear();
            nodesRead.clear();
        }
...
// node's parent.
        ...
        forceinline data_node_type *get_leaf_psql(IndexScanDesc scan, T key) {
...
//                std::cout << "children_: " << node->children_ << std::endl;
//                std::cout << "children_[bucketID]: " << node->children_[bucketID] << std::endl;
                // 这里开始，对新 cur 的读取得从磁盘中进行了，而不是直接读内存
                if (node->children_[bucketID] == nullptr) {
                    node->children_[bucketID] = readNode(scan, node->childrenBlockNumber[bucketID]);
                }
                cur = node->children_[bucketID];
                if (cur->is_leaf_) {
                    stats_.num_node_lookups += cur->level_;
                    ...
                    if (std::abs(bucketID_prediction - bucketID_prediction_rounded) <=
                        ...
                        if (bucketID_prediction_rounded <= bucketID_prediction) {
                            ...
                            if (leaf->prevLeafBlockNumber != InvalidBlockNumber) {
                                leaf->prev_leaf_ = static_cast<data_node_type *>(readNode(scan,
                                                                                          leaf->prevLeafBlockNumber));
                            }

                            if (leaf->prev_leaf_ && leaf->prev_leaf_->last_key_psql(scan) >= key) {
                                return leaf->prev_leaf_;
                            }

                        } else {

                            if (leaf->nextLeafBlockNumber != InvalidBlockNumber) {
                                leaf->next_leaf_ = static_cast<data_node_type *>(readNode(scan,
                                                                                          leaf->nextLeafBlockNumber));
                            }

                            if (leaf->next_leaf_ && leaf->next_leaf_->first_key_psql(scan) <= key) {
                                return leaf->next_leaf_;
                            }
    ...
    private:
        ...
        inline void correct_traversal_path(data_node_type *leaf,
                                           ...
                                           bool left) const {
//            std::cout << "correct_traversal_path" << std::endl;
            if (left) {
    ...
    public:
        ...
        typename self_type::Iterator find_psql(IndexScanDesc scan, const T &key) {
//            std::cout << "find_psql, key = " << key << std::endl;
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf_psql(scan, key);
//            std::cout << "leaf = " << leaf << std::endl;
        ...
        typename self_type::Iterator lower_bound_psql(IndexScanDesc scan, const T &key) {
//            std::cout << "===lower_bound_psql" << std::endl;
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf_psql(scan, key);
            int idx = leaf->find_lower(key);
            return Iterator(leaf, idx);  // automatically handles the case where idx ==
        ...
        typename self_type::Iterator upper_bound_psql(IndexScanDesc scan, const T &key) {
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf_psql(scan, key);
            int idx = leaf->find_upper(key);
            return Iterator(leaf, idx);  // automatically handles the case where idx ==
